...
class DoubleConvResidual(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=None):
        ...
        self.conv1 = SingleConv(in_channels, mid_channels, kernel_size=3, padding=CONV_PAD)
        self.conv2 = SingleConv(in_channels, out_channels, kernel_size=3, padding=CONV_PAD)
...
class UpConv(nn.Module):
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.conv_1x1_half_channels = SingleConv(in_channels, in_channels // 2)
            self.conv_1x1 = SingleConv(in_channels, in_channels)
        else:
            self.upsample = nn.ConvTranspose2d(
                in_channels, in_channels // 2, kernel_size=2, stride=2
            )
        self.double_conv_3x3 = DoubleConv(in_channels, out_channels)

    def forward(self, decoder_layer, encoder_layer):
        encoder_layer = self.adjust_encoder_layer(encoder_layer)
        decoder_layer = self.adjust_decoder_layer(decoder_layer, encoder_layer)
        x = torch.cat([encoder_layer, decoder_layer], dim=1)
        x = self.double_conv_3x3(x)
        return x

    def adjust_decoder_layer(self, decoder_layer, encoder_layer):
        decoder_layer = self.conv_1x1_half_channels(decoder_layer)
    ...
    def adjust_encoder_layer(self, encoder_layer):
        encoder_layer = self.conv_1x1(encoder_layer)
...
class UNetValid3(nn.Module):
    def __init__(self, in_channels, n_classes, bilinear=False):
        super(UNetValid3, self).__init__()
...
if __name__ == "__main__":
    ...
    model = UNetValid3(in_channels=3, n_classes=1, bilinear=True)
    preds = model(x)


# Changelog
# - Added residual connections in double conv residual
# - added 1x1 conv on the encoder layer before concat with the upscaled decoder layer