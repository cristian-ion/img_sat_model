...
from solution.classification.dataset import ImgClsDataset
...
class ImgClsTrain:
    def __init__(self, configpath) -> None:
        ...
        model.fc = torch.nn.Linear(
            model.fc.in_features, config["model"]["output"]["num_classes"]
        )
        ...
        with open(
            os.path.join(self.location, f"{self.name}_config.yaml"), "w"
        ) as stream:
            try:
